...
window.LArea =(function(){
    ...
    MobileArea.prototype = {
        ...
        bindEvent: function() {
            ...
            function popupArea(e) {
                ...
                _self.gearArea.innerHTML = '<div class="area_ctrl slideInUp">' +
                    ...
                    '<div class="area_roll">' +
                    '<div>' +
                    '<div class="gear area_province" id="province" data-areatype="area_province"></div>' +
                    '<div class="area_grid">' +
                    '</div>' +
                    '</div>' +
                    '<div>' +
                    '<div class="gear area_city" id="city" data-areatype="area_city"></div>' +
                    '<div class="area_grid">' +
                    '</div>' +
                    '</div>' +
                    '<div>' +
                    '<div class="gear area_county" id="county" data-areatype="area_county"></div>' +
                    '<div class="area_grid">' +
                    '</div>' +
                    '</div>' +
                    '</div>' +
            ...
            function areaCtrlInit() {
                ...
                // :TODO 设置初始化值，dom获取有问题
                // if(_self.type === 1){
                //     if(_self.trigger.valueto){
                //         let AreaDefault = _self.trigger.valueto.split(',');
                //         for(let k in _self.data){
                //             if(_self.data[k].id === AreaDefault[0]){
                //                 _self.value[0] = k;
                //             }
                //         }
                //         for(let k in _self.data){
                //             if(_self.data[k].id === AreaDefault[1]){
                //                 _self.value[1] = k;
                //             }
                //         }
                //         for(let k in _self.data){
                //             if(_self.data[k].id === AreaDefault[2]){
                //                 _self.value[2] = k;
                //             }
                //         }
                //     }
                ...
                _self.gearArea.querySelector(".area_province").setAttribute("val", _self.value[1]);
        ...
        finish: function(e) {
            ...
            if (_self.params.finish && typeof _self.params.finish === 'function') {
                _self.trigger.value = addrText;
                _self.value = [provinceVal, cityVal, countyVal];
                _self.params.finish(addrText, addrCode, [provinceVal, cityVal, countyVal]);
            } else {
                addrText = addrText.replace(/县,|市辖区,/ig, '').replace(/,/ig, ''); // 直辖市结果处理
                _self.trigger.value = addrText;
                _self.value = [provinceVal, cityVal, countyVal];
                if(_self.valueTo){
                    _self.valueTo.value = addrCode;
                }
            }
